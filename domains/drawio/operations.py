...
def _fn_place_shape(ui_graph: Dict[str, Any], tool_name: str) -> dict:
    ...
    print(f"  [L1] place_shape('{tool_name}') → click ({x}, {y}) + Enter")
    atom_click_at(x, y)
    time.sleep(0.3)
    atom_press("Return")
...
def _fn_resize_shape(
    ...
    print(f"  [L1] resize_shape('{direction}', {amount}) → "
...
def _fn_extend_shape(ui_graph: Dict[str, Any], direction: str) -> dict:
    ...
    print(f"  [L1] extend_shape('{direction}') → drag ({sx},{sy}) → ({tx},{ty})")
    atom_drag(sx, sy, tx, ty)
    time.sleep(0.8)
    handles = _refresh_handles(ui_graph)
    new_obj_id = None
...
def _fn_rotate_shape(ui_graph: Dict[str, Any], angle_degrees: float) -> dict:
    ...
    print(f"  [L1] rotate_shape({angle_degrees}°) → "
...
def _fn_move_shape(
    ...
    print(f"  [L1] move_shape('{direction}', {amount}) → "
...
def _fn_hover_object(ui_graph: Dict[str, Any], object_id: str) -> dict:
    ...
    cx, cy = bx + bw // 2, by + bh // 2
    print(f"  [L1] hover_object('{object_id}') → moveTo ({cx},{cy})")
    atom_move_to(cx, cy)
...
def _fn_connect_shapes(
    ...
    print(f"  [L1] connect_shapes({source_id}→{target_id}) "
